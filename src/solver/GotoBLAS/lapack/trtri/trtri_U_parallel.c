...
blasint CNAME(blas_arg_t *args, BLASLONG *range_m, BLASLONG *range_n, FLOAT *sa, FLOAT *sb, BLASLONG mypos) {
  ...
  for (i = 0; i < n; i += blocking) {
    ...
    gemm_thread_m(mode, &newarg, NULL, NULL, TRSM, sa, sb, blas_cpu_number);
    ...
    gemm_thread_n(mode, &newarg, NULL, NULL, GEMM_NN, sa, sb, blas_cpu_number);
    ...
    gemm_thread_n(mode, &newarg, NULL, NULL, TRMM, sa, sb, blas_cpu_number);
