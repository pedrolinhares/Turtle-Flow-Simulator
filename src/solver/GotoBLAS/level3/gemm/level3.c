...
int CNAME(blas_arg_t *args, void *range_m, void *range_n,
  ...
  BLASULONG innercost  = 0;
  BLASULONG outercost  = 0;
  BLASULONG kernelcost = 0;
  double total;
#endif
...
#if 0
  fprintf(stderr, "GEMM: M_from : %ld  M_to : %ld  N_from : %ld  N_to : %ld  k : %ld\n", m_from, m_to, n_from, n_to, k);
  ...
  printf( " Total %10.3f%%  %10.3f MFlops\n",
	  ((double)(m_to - m_from) * (double)(n_to - n_from) * (double)k) / (double)kernelcost * 100 * COMPSIZE * COMPSIZE,
	  2200. * (2. * (double)(m_to - m_from) * (double)(n_to - n_from) * (double)k) / (double)kernelcost * COMPSIZE * COMPSIZE);
