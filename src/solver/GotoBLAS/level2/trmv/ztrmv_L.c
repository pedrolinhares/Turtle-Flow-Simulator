...
int CNAME(BLASLONG m, FLOAT *a, BLASLONG lda, FLOAT *b, BLASLONG incb, void *buffer){
  ...
  for (is = m; is > 0; is -= TRSV_P1){
    ...
    if (m - is > 0){
#if   TRANSA == 1
      ZGEMV_N(m - is, min_i, 0, dp1, ZERO,
	      a + (is + (is - min_i) * lda) * 2, lda,
	      B + (is - min_i) * 2, 1,
	      B + is * 2, 1, gemvbuffer);
#else
      ZGEMV_R(m - is, min_i, 0, dp1, ZERO,
	      a + (is + (is - min_i) * lda) * 2, lda,
	      B + (is - min_i) * 2, 1,
	      B + is * 2, 1, gemvbuffer);
#endif
    ...
    for (i = 0; i < min_i; i++) {
...
#if (TRANSA == 1) || (TRANSA == 3)
#if TRANSA == 1
      if (i > 0) ZAXPY_K (i, 0, 0, BB[0], BB[1], AA + 2, 1, BB + 2, 1, NULL, 0);
#else
      if (i > 0) ZAXPYC_K(i, 0, 0, BB[0], BB[1], AA + 2, 1, BB + 2, 1, NULL, 0);
#endif
#endif
      ...
      if (i < min_i - 1) {
#if   TRANSA == 2
	temp[0] = ZDOTU_K(min_i - i - 1, AA - (min_i - i - 1) * 2, 1, BB - (min_i - i - 1) * 2, 1);
#else
	temp[0] = ZDOTC_K(min_i - i - 1, AA - (min_i - i - 1) * 2, 1, BB - (min_i - i - 1) * 2, 1);
#endif
	GET_IMAGE(temp[1]);
    ...
    if (is - min_i > 0){
#if TRANSA == 2
      ZGEMV_T(is - min_i, min_i, 0, dp1, ZERO,
	      a + (is - min_i) * lda * 2, lda,
	      B,              1,
	      B + (is - min_i) * 2, 1, gemvbuffer);
#else
      ZGEMV_C(is - min_i, min_i, 0, dp1, ZERO,
	      a + (is - min_i) * lda * 2, lda,
	      B,              1,
	      B + (is - min_i) * 2, 1, gemvbuffer);
#endif
