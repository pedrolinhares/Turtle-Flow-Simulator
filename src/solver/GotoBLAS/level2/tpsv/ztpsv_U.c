...
int CNAME(BLASLONG m, FLOAT *a, FLOAT *b, BLASLONG incb, void *buffer){
  ...
  for (i = 0; i < m; i++) {
    ...
    if (i > 0) {
#if TRANSA == 2
      result[0] = ZDOTU_K(i, a + 2, 1, B + (m - i) * 2, 1);
#else
      result[0] = ZDOTC_K(i, a + 2, 1, B + (m - i) * 2, 1);
#endif
      GET_IMAGE(result[1]);
      
      B[(m - i - 1) * 2 + 0] -= result[0];
      B[(m - i - 1) * 2 + 1] -= result[1];
    }
#endif
    ...
    if (i < m - 1) {
#if TRANSA == 1
      ZAXPY_K (m - i - 1, 0, 0, - B[(m - i - 1) * 2 + 0], -B[(m - i - 1) * 2 + 1],
	       a - (m - i - 1) * COMPSIZE, 1, B, 1, NULL, 0);
#else
      ZAXPYC_K(m - i - 1, 0, 0, - B[0], -B[1],
	       a - (m - i - 1) * COMPSIZE, 1, B, 1, NULL, 0);
#endif
    }
#endif
