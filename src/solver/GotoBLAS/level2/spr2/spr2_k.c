...
int CNAME(BLASLONG m, FLOAT alpha_r, FLOAT *x, BLASLONG incx, 
  ...
  for (i = 0; i < m; i++){
#ifndef LOWER
    AXPY_K(i + 1, 0, 0, alpha_r * X[i], Y,     1, a, 1, NULL, 0);
    AXPY_K(i + 1, 0, 0, alpha_r * Y[i], X,     1, a, 1, NULL, 0);
    a += i + 1;
#else
    AXPY_K(m - i, 0, 0, alpha_r * X[i], Y + i, 1, a, 1, NULL, 0);
    AXPY_K(m - i, 0, 0, alpha_r * Y[i], X + i, 1, a, 1, NULL, 0);
    a += m - i;
#endif
    }
