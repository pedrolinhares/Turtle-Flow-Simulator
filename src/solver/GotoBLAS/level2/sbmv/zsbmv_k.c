...
int CNAME(BLASLONG n, BLASLONG k, FLOAT alpha_r, FLOAT alpha_i,
  ...
  for (i = 0; i < n; i++) {
    ...
    if (length > 0) {
      temp[0] = ZDOTU_K(length, a + COMPSIZE, 1, X + (i + 1) * COMPSIZE, 1);
      GET_IMAGE(temp[1]);
      Y[i * 2 + 0] += alpha_r * temp[0] - alpha_i * temp[1];
      Y[i * 2 + 1] += alpha_r * temp[1] + alpha_i * temp[0];
      }
