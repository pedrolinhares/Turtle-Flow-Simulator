...
CGrid1d::CGrid1d(CDataControl *_dcontrol) :CGrid(_dcontrol)
{
    ...
    data = dcontrol->DataCGrid1d("kernel/ArqRes1d.dat"); ///< Getting Grid data;
    ...
    block = new CBlock[blknumber];
     ...
     for(int i = 0; i < blknumber ; i++) {  
           ...
           data = dcontrol->DataCBlock("kernel/ArqBlock.dat", i); ///< Getting data of Block i;          
   ...
   switch (fluidtype1) {
   	case 1: { ///< Case 1 represents a water simulation
	   		data = dcontrol->DataCFluid("kernel/ArqWater.dat"); ///< Getting the water data;
	   		if (data == NULL) { 
				cout << "There is no water data." << endl;  
				cin.ignore();
			}
   	...
   	case 2: {
	   		data = dcontrol->DataCFluid("kernel/ArqOil.dat"); ///< Getting the oil data;
	   		if (data == NULL) { 
				cout << "There is no oil data." << endl;  
				cin.ignore();
			}
   	...
   	case 3: {
   			data = dcontrol->DataCFluid("kernel/ArqGas.dat");  ///< Getting the gas data;
   	  		if (data == NULL) { 
				cout << "There is no gas data." << endl;  
				cin.ignore();
			}
   ...
   for( int i=0; i < cellnumber ; i++ ) {
		...
		data = dcontrol->DataCell1d("kernel/ArqCell1d.dat", i);  ///< Getting the data of the cell i;
   ...
   data = dcontrol->DataCWell1d("kernel/ArqWell1d.dat"); ///< Getting well data;
...
void CGrid1d::SetBoundConditions() {
	...
	double *data;
    data = dcontrol->DataBoundCond1d("kernel/ArqBoundCond1d.dat"); ///< Getting Boundary Conditions data;
...
void CGrid1d::InitiateSolution() {
	...
	double *data;
    data = dcontrol->DataInitiate1d("kernel/ArqInit1d.dat"); ///< Getting the initialization data;
